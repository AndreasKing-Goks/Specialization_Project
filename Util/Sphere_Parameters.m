...
%% General Parameters
% Environment
Param.rho_fluid = 1000;      % Fresh water density, kg/m3
Param.mu_fluid = 1.002e-3;   % Fresh water dynamic viscosity, Pa.s
Param.g = 9.81;              % m/s2, Positive pointing down to center of Earth

% Sphere Properties
Param.D = 0.25;                                     % Diameter, m
Param.R = Param.D/2;                                % Radius, m
Param.V = 4/3 * pi * Param.R^3;                     % m3
...
Param.foam_height = Param.percent_foam * Param.R;
Param.metal_height = Param.percent_metal * Param.R;
Param.c_t_height = Param.R - Param.foam_height;
Param.c_b_height = Param.R - Param.metal_height;

Param.V_foam = pi * (Param.foam_height^2) *(Param.R - Param.foam_height/3);
Param.V_metal = pi * (Param.metal_height^2) *(Param.R - Param.metal_height/3);

% Param.V_foam = pi*((Param.R^3 - Param.R * Param.c_t_height^2) - ((Param.R^3 - Param.c_t_height^3)/3));
% Param.V_metal = pi*((Param.R^3 - Param.R * Param.c_b_height^2) - ((Param.R^3 - Param.c_b_height^3)/3));

Param.V_c_t = (Param.V/2) - Param.V_foam;
Param.V_c_b = (Param.V/2) - Param.V_metal;
Param.V_cavity = Param.V_c_t + Param.V_c_b;
...
Param.ma = -Param.rho_fluid * 2/3 * pi * Param.R^3;         % Added mass, kg
...
Param.centroid_cavity_t = ((Param.centroid_HS * Param.V_HS) - (Param.centroid_foam * Param.V_foam))/Param.V_c_t;
Param.centroid_cavity_b = -((Param.centroid_HS * Param.V_HS) - (Param.centroid_metal * Param.V_metal))/Param.V_c_b;
Param.centroid_cavity = ((Param.centroid_cavity_t * Param.V_c_t) + (Param.centroid_cavity_b * Param.V_c_b))/(Param.V_cavity);
...
Param.rg_o = Param.rg - Param.ro; % Distance from center of gravity to center of origin 
Param.rb_o = Param.rb - Param.ro; % Distance from center of buoyancy to center of origin
